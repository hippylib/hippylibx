# Values in m_true remain consistent before and during function call solveFwd but is filled with overflows after the function call.
...
class PDEVariationalProblem:
    ...
    def solveFwd(self, state, x): #state is a vector
        ...
        # print("inside function - 1\n") #Both values as expected
        # print(mfun.vector[:].min())
        # print(mfun.vector[:].max(),'\n')
...
# m_true_expr = dl.Expression("std::log(0.01) + 3.*( ( ( (x[0]-2.)*(x[0]-2.) + (x[1]-2.)*(x[1]-2.) ) < 1.) )", degree=2)
...
#M-1 This does copy all values in m_true
m_true2 = dlx.fem.Function(Vh_m)
m_true2.x.array[:] = m_true[:]
...
print("after function call\n") #Both values correct.
print(m_true2.vector[:].min())
print(m_true2.vector[:].max(),'\n')
