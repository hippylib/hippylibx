...
class VariationalRegularization:
    def __init__(self, Vh : dlx.fem.FunctionSpace, functional_handler, isQuadratic=False):
        ...
        self.petsc_options_R = {"ksp_type": "cg", "pc_type": "gamg", "ksp_rtol":"1e-12", "ksp_max_it":"1000", "ksp_error_if_not_converged":"true", "ksp_initial_guess_nonzero":"false"}
        ...
        self.M = dlx.fem.petsc.assemble_matrix( dlx.fem.form( ufl.inner(self.mtrial, self.mtest)*self.dx ) )
        self.M.assemble()
    ...
    def grad(self, m: dlx.la.Vector, out: dlx.la.Vector) -> dlx.la.Vector:
        ...
        dlx.fem.petsc.assemble_vector( tmp_out, dlx.fem.form(ufl.derivative( self.functional_handler(self.mfun), self.mfun, self.mtest))  )
