...
def LS_ParameterList():
    ...
    parameters["max_backtracking_iter"] = [
        10,
...
def TR_ParameterList():
    ...
    parameters["eta"] = [
        0.05,
        "Reject step if (actual reduction)/(predicted reduction) < eta",
    ]
...
def ReducedSpaceNewtonCG_ParameterList():
    ...
    parameters["rel_tolerance"] = [
        1e-6,
        "we converge when sqrt(g,g)/sqrt(g_0,g_0) <= rel_tolerance",
    ]
    ...
    parameters["globalization"] = [
        "LS",
        "Globalization technique: line search (LS)  or trust region (TR)",
    ]
    ...
    parameters["cg_coarse_tolerance"] = [
        0.5,
    ...
    parameters["LS"] = [
        LS_ParameterList(),
        "Sublist containing LS globalization parameters",
    ]
    parameters["TR"] = [
        TR_ParameterList(),
        "Sublist containing TR globalization parameters",
    ]
...
class ReducedSpaceNewtonCG:
    ...
    def _solve_tr(self, x):
        ...
        while (self.it < max_iter) and (not self.converged):
            ...
            if print_level >= 0:
                print(
                    "{0:3d} {1:3d} {2:15e} {3:15e} {4:15e} {5:15e} {6:14e} {7:14e} {8:14e} {9:11} {10:14e}".format(
                        self.it,
                        HessApply.ncalls,
                        cost_old,
                        misfit_old,
