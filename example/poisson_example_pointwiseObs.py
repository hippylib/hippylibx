...
def run_inversion(
    ...
    alpha = 100.0
    f = 1.0
    pde_handler = PoissonProblem(alpha, f)
    ...
    # LIKELIHOOD
    n_obs = 50
    obs_points = np.random.rand(n_obs, 2)
    B = hpx.pointwiseInterpolationMatrix(Vh_phi, obs_points)
    data = B.createVecLeft()
    B.mult(u_true.petsc_vec, data)
    hpx.parRandom.normal_perturb(np.sqrt(noise_variance), data)
    misfit = hpx.DiscreteStateObservationMisfit(B, data, noise_variance)
    ...
    with dlx.io.VTXWriter(
        ...
        [m_fun, m_true_fun, u_map_fun, u_true_fun],
    ) as vtx:
        vtx.write(0.0)
