...
def run_inversion(
    ...
    with dlx.io.VTXWriter(
        msh.comm,
        "poisson_Robin_Variational_Regularization_prior_np{0:d}_Prior.bp".format(nproc),
    ...
    with dlx.io.VTXWriter(
        msh.comm,
        "poisson_Robin_prior_Variational_Regularization_samples_prior_np{0:d}.bp".format(
            nproc
        ),
        prior_samples,
    ) as vtx:
    ...
    with dlx.io.VTXWriter(
        msh.comm,
        "poisson_Robin_prior_Variational_Regularization_samples_posterior_np{0:d}.bp".format(
            nproc
        ),
        posterior_samples,
    ) as vtx:
